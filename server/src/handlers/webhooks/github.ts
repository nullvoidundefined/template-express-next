  ...
  try {
    if (event === "push") {
      ...
      const matching = services.filter(
        (s) =>
          s.github_repo === repoName && s.github_owner === repoOwner && s.github_branch === branch,
      );

      for (const service of matching) {
        await upsertGithubStatus(service.id, {
          last_commit_sha: payload.after?.slice(0, 7) ?? null,
          last_commit_message: payload.head_commit?.message?.split("\n")[0] ?? null,
    ...
    if (event === "workflow_run") {
      ...
      for (const service of matching) {
        ...
        let workflowStatus: "success" | "failure" | "pending" | "cancelled" | null = null;
