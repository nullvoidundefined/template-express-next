...
describe("services handlers", () => {
  ...
  describe("listServices", () => {
    ...
    it("returns 500 when repo throws", async () => {
      vi.mocked(servicesRepo.listServices).mockRejectedValueOnce(new Error("DB error"));
      const res = await request(app).get("/services");
  ...
  describe("createService", () => {
    ...
    it("returns 400 for invalid URL", async () => {
      const res = await request(app).post("/services").send({ name: "Test", url: "not-a-url" });
      expect(res.status).toBe(400);
    });
    ...
    it("returns 500 when repo throws", async () => {
      vi.mocked(servicesRepo.createService).mockRejectedValueOnce(new Error("DB error"));
      const res = await request(app)
        .post("/services")
  ...
  describe("updateService", () => {
    it("returns 400 for invalid UUID", async () => {
      const res = await request(app).put("/services/not-a-uuid").send({ name: "New" });
      expect(res.status).toBe(400);
      expect(res.body.error.message).toBe("Invalid service ID");
    });
    ...
    it("returns 404 when not found", async () => {
      ...
      const res = await request(app).put(`/services/${id}`).send({ name: "New" });
      expect(res.status).toBe(404);
    });
    ...
    it("returns 400 for invalid body fields", async () => {
      const res = await request(app).put(`/services/${id}`).send({ url: "not-a-url" });
      expect(res.status).toBe(400);
    });
  });
