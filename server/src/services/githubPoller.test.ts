...
describe("pollGitHub", () => {
  ...
  it("populates commit data from GitHub API response", async () => {
    ...
    fetchMock.mockResolvedValueOnce(commitResponse).mockResolvedValueOnce(runsResponse);
  ...
  it("populates workflow data from GitHub API response", async () => {
    ...
    expect(data.workflow_run_url).toBe("https://github.com/owner/repo/actions/runs/123");
  });
  ...
  it("skips polling when rate limit nearly exhausted", async () => {
    const commitResponse = makeResponse(
      [{ sha: "abc", commit: { message: "msg", author: { name: "user", date: "2024-01-01" } } }],
