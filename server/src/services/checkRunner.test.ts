...
describe("determineStatus", () => {
  ...
  it("returns 'degraded' when response time > 80% of timeout", () => {
    const result = determineStatus({ ...base, responseTimeMs: 8500, timeoutMs: 10000 });
    expect(result).toBe("degraded");
  });

  it("returns 'up' when response time is exactly 80% of timeout", () => {
    const result = determineStatus({ ...base, responseTimeMs: 8000, timeoutMs: 10000 });
  ...
  it("returns 'down' when TLS is invalid (expired)", () => {
    ...
    const result = determineStatus({ ...base, tlsValid: false, tlsExpiresAt: expired });
