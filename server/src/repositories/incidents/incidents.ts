...
export async function listIncidentsByService(serviceId: string): Promise<Incident[]> {
...
export async function getIncidentById(id: string): Promise<Incident | null> {
  const result = await query<Incident>(`SELECT * FROM incidents WHERE id = $1`, [id]);
...
export async function createIncident(
  ...
  const result = await query<Incident>(
    `INSERT INTO incidents (service_id, title, cause, status, started_at)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [serviceId, data.title, data.cause ?? null, data.status, data.started_at ?? new Date()],
  );
