...
The client directory structure (web client and shared packages) is documented in `../CLAUDE.md`. This section covers the web-specific layout only.
...
- **Named exports only.** See root CLAUDE.md rule 5 for the canonical statement. Exceptions for this workspace: Next.js App Router convention files (`page.tsx`, `layout.tsx`, etc.) require `export default`; Storybook `*.stories.tsx` files and `.storybook/` config files may also use `export default`. All other files use named exports only.
...
Access values: `'public'` (no auth required), `'private'` (must have `sid` cookie), `'admin'` (must have `sid` cookie; role is verified server-side).

Middleware only checks cookie presence. It never verifies the session or the user's role. Authorization is enforced server-side in two places:

1. The `(protected)` layout server component calls `/auth/me` and redirects unauthenticated users to `/login`.
2. The `(admin)` layout server component (`app/(protected)/admin/layout.tsx`) calls `/auth/me` and redirects authenticated non-admins (`user.role !== 'admin'`) to `/dashboard`.
3. Admin API routes are protected by the server's `requireAdmin` middleware, which is the real security boundary; the layout redirects are UX only.
...
3. Admin section: add `{ prefix: '/your-section', access: 'admin' }` to `PREFIX_RULES`, place pages under `app/(protected)/admin/`, and guard the API routes with `requireAdmin`.
...
- All backend application routes are versioned under `/v1`. The proxy maps `/api/*` to `/v1/*` (so `/api/auth/me` reaches the server's `/v1/auth/me`), and `services/api.ts` applies the `/v1` prefix once so callers pass unversioned paths (`/auth/me`). Health checks (`/health`, `/health/ready`) stay at root.
- Client-side code still uses `NEXT_PUBLIC_API_URL` through `services/api.ts`.
- **Always set `INTERNAL_API_URL` on the deployed web service.** Server components, layouts (`(protected)`, `(admin)`), and the proxy fall back to `NEXT_PUBLIC_API_URL` when it is unset, which routes server-to-server traffic over the public domain (slower, external egress, and can fail when the public URL sits behind a proxy/CDN). Point it at the API's `*.railway.internal` address.

### Storybook

Every component in `src/components/` requires a co-located `*.stories.tsx` file created or updated in the same commit as the component. `export default` is allowed in story files and `.storybook/` config files (see root CLAUDE.md rule 5 for the full exception list).
