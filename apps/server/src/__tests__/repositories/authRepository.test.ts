...
const mockWithTransaction = vi.fn(
  (fn: (client: PoolClient) => Promise<unknown>) => fn(mockClient),
);

// Inject fakes rather than mocking the pool module (drops vi.mock of the DB).
const authRepo = createAuthRepo({
  query: mockQuery,
  withTransaction: mockWithTransaction,
} as unknown as AuthRepoDeps);
...
describe('auth repository', () => {
  ...
  it('authenticate runs bcrypt.compare even when the user is missing (constant time)', async () => {
    mockQuery.mockResolvedValueOnce(mockResult([]));
    await authRepo.authenticate('nobody@example.com', 'pwd');
    // The missing-user path must still hash-compare so its timing matches the
    // wrong-password path, preventing user enumeration.
    expect(vi.mocked(bcrypt.compare)).toHaveBeenCalled();
  });
