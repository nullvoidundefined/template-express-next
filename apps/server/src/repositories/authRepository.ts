 ...
 * without mocking the pool module. Mirrors the signatures exported by
 * `app/database/databasePool.js`.
 */
interface AuthRepoDeps {
...
/**
 * Precomputed bcrypt hash compared against on the missing-user login path so a
 * request for an unregistered email takes the same time as a wrong-password
 * request. Prevents user enumeration via response timing.
 */
const TIMING_EQUALIZER_HASH =
  '$2b$12$/xGDf2LDK8aBFqfMhoGPt.0trFNim..xQQi02u4wmAYUsyYlZslsa';

function createAuthRepo({ query, withTransaction }: AuthRepoDeps) {
  ...
  async function getSessionWithUser(sessionId: string): Promise<User | null> {
  ...
  async function authenticate(
    ...
    if (!row) {
      // Equalize timing with the wrong-password path so an unregistered email is
      // indistinguishable from a wrong password (prevents user enumeration).
  ...
  async function updateUser(
    ...
    values.push(userId);
    const result = await query<User>(
      `UPDATE users SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING id, email, role, created_at, updated_at`,
      values,
    );
    const row = result.rows[0];
...
type AuthRepo = ReturnType<typeof createAuthRepo>;

export { createAuthRepo };
export type { AuthRepo, AuthRepoDeps };
