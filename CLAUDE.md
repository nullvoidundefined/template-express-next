...
- `apps/client/CLAUDE.md` for shared React and TypeScript conventions for the web client (auto-loaded when working in `apps/client/`)
...
- `packages/types/CLAUDE.md` for shared TypeScript types used by server and web client
...
4. **File names encode their type as a camelCase suffix.** Every TypeScript/JavaScript source file (except the exemptions below) must include its architectural type as a suffix in the filename. Use camelCase with no dots or hyphens. Examples: `authHandler.ts`, `postsRepository.ts`, `hashService.ts`, `stripeClient.ts`, `authRoutes.ts`, `authSchema.ts`, `authConstants.ts`, `useAuthHook.ts`, `apiService.ts`. Exemptions: Next.js App Router convention files (`page.tsx`, `layout.tsx`, `route.ts`, `middleware.ts`, etc.), Storybook story files, web Components (e.g. `Button.tsx`), entry point files (`index.ts`, `app.ts`, `server.ts`, `worker.ts`), TypeScript declaration files (`*.d.ts`), and config/tooling files at workspace roots.
5. **Alphabetical ordering is mandatory.** Type definitions, type keys, object keys, JSX props, destructured props, union type members, and imports within each group are all alphabetized. This removes decision fatigue, eliminates merge conflicts from arbitrary reordering, and makes scanning faster.
6. **Named exports only across the whole codebase.** Never `export default`, in either the frontend or backend. Every file ends with an explicit `export { Name }` statement. Exceptions: Next.js App Router convention files (`page.tsx`, `layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx`, `template.tsx`, `route.ts`) require `export default` and are exempt; Storybook `*.stories.tsx` files and `.storybook/` config files may also use `export default`.
...
8. **Shared types live in `packages/types/`.** Any TypeScript type that is used by both the server and web client must be defined in `@repo/types` and imported from there. Never duplicate a type definition across workspaces. If a type starts in one workspace and the other workspace needs it, move it to `packages/types/` immediately.
9. **Test files live in `__tests__/`, never beside source files.** Every workspace keeps its tests in a `src/__tests__/` directory that mirrors the source tree. No `Component.test.tsx` next to `Component.tsx`. See the Testing Conventions section. This overrides any co-location guidance found in workspace-level CLAUDE.md files.
10. **Every feature ships a complete checklist.** Before closing any user-facing feature: add a row to `docs/feature-list/features.md`; create or update a user story in `docs/user-stories/`; a Playwright E2E spec in `e2e/` must exist; squash merge onto `main`; delete shipped specs and plans from `docs/superpowers/`.
11. **Squash merge all feature branches.** `git merge --squash` only. Never a regular merge or rebase. One commit per feature on `main`.
12. **`fix:` commits require a test file.** If the commit subject starts with `fix:`, `bug:`, `bugfix:`, or `hotfix:`, at least one test file must be staged. Enforced by lefthook. Infrastructure changes use `chore:`, not `fix:`.
13. **PostGIS from day one** (when geographic data is needed). Never store lat/lng as plain floats and do math in application code. The `geography` or `point` column type ships in the first migration that needs it.
14. **node-pg-migrate default values: bare strings, never double-quoted.** `default: 'active'` not `default: "'active'"`. The builder API adds its own quoting; passing a pre-quoted string produces triple-quoted DDL and causes CHECK constraint failures on INSERT. Enforced by lefthook.
15. **The `set_updated_at` trigger is created once and reused.** Define the trigger function in the users migration. Every subsequent table wires it up with `CREATE TRIGGER ... EXECUTE FUNCTION set_updated_at()`. Never redefine the function.
16. **Analytics events as typed constants, no magic strings.** All PostHog event names live in `packages/constants/src/analytics.ts` and are imported from there. Never pass a raw string to `posthog.capture()`.
17. **PostHog events route through a reverse proxy.** Use an opaque path matching the product vocabulary (never `/analytics`, `/tracking`, `/posthog`, or `/telemetry`). Configure before launch; retroactive setup loses early funnel data.
18. **Environment validation at startup.** Every server calls `validateEnv()` before any middleware registration. A server that boots with missing env vars and fails on the first request is harder to debug than one that crashes immediately.
19. **Health endpoints on every API service.** `/health` (liveness, always 200) and `/health/ready` (readiness, verifies DB connectivity). Register before all application routes.
20. **`NODE_ENV` must always be explicitly set.** Every Railway service has `NODE_ENV=production`. Never leave it unset or defaulted.
21. **SSL: never `rejectUnauthorized: false`.** Use the conditional pattern gated on `isProduction()`. This was left in a prior project for weeks and silently disabled certificate validation in production.
22. **CORS: never wildcard with credentials.** `CORS_ORIGIN` must be the exact stable Railway production URL. Never a preview or ephemeral URL.
23. **CSRF.** See rule 7.
24. **Session cookies: correct `SameSite` and `Secure` flags.** `SameSite: 'lax'` when frontend and backend share the same Railway domain. `SameSite: 'none'` + `secure: true` only when running on separate domains. `httpOnly: true` always.
25. **Per-component folder structure.** Every component lives in `ComponentName/ComponentName.tsx` plus `ComponentName.module.scss`. Never a flat `.tsx` directly under `components/`.
26. **`displayName` and `data-test-id` on every component.** Set `ComponentName.displayName = 'ComponentName'` immediately after the function definition. Outermost DOM element has `data-test-id` in kebab-case.
27. **State management by layer.** See State Management table below.
28. **TanStack Query for all server state.** No raw `useEffect` + `fetch` in components. All API calls go through `useQuery` and `useMutation`.
29. **Cloudflare R2 for all image storage.** Store keys in the database, not full URLs. Derive the public URL at read time. Validate file size and MIME type on the server before upload.
30. **Sentry on both server and client from day one.** Initialize before any business logic. Set user context in `loadSession`. Upload source maps in CI on every production deploy.
31. **Commit `pnpm-lock.yaml` with every `package.json` change.** CI uses `--frozen-lockfile`.
32. **Build runs on pre-push.** The lefthook `pre-push` hook runs `pnpm build`. A broken build never lands on `main`.

---

## Stack

- **Monorepo:** pnpm workspaces. `apps/server/` (Express 5 + TypeScript), `apps/client/web/` (Next.js 15), `packages/tokens/` (shared design tokens), `packages/types/` (shared TypeScript types), `packages/constants/` (shared constants including analytics event names).
...
- **Design tokens:** `packages/tokens/` is the single source of truth for all visual values. The web client consumes `dist/_tokens.scss` (CSS custom properties).
...
- The path inside `__tests__/` matches the path inside `src/` exactly. `src/handlers/postsHandler.ts` has its test at `src/__tests__/handlers/postsHandler.test.ts`.
